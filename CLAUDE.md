...
- `app/home/page.tsx` — dashboard (scoreboard folded, **Vs the house** folded, Achievements folded, Completed log folded under Weekly Progress). Week cards / **Completed** open that log. Bonus flag + unique-week tally when the week has a bonus day (`components/BonusFlag.tsx`). Stat cards + progress charts show you / household avg of people who finished a workout in the last 7 days, **including you**, minus Test (`GET /api/stats` `household`; null if nobody trained). Week progress caps at 4. **Vs the house** (`GET /api/exercise-compare?period=7|30|all`): current athlete only; Best day / Total weight ranking plus two boards (best-day weight, best-day reps); hidden for Test. Enjoyment charts if the athlete has session ratings. Scoreboard honor roll from `GET /api/scoreboard` (`householdBonusHonor`)
- `app/workout/page.tsx` — Select + logging. Finish and quit require 1–5 stars (`POST /api/session-ratings`). Exercise thumbs via `ExerciseThumbs`. Finished days share `CompletedSessionCard` with the Home log. Bonus days get Extra credit copy + rest-between-uppers hint. Finish PUT returns `{ bonus, bonusCount }` for the takeover (`bonus_complete` lines)
...
- `app/api/*/route.ts` — one file per resource (`GET /api/scoreboard?period=7|30|all` includes honor roll, `GET /api/stats` (includes `household`), `GET /api/exercise-compare?period=7|30|all` (current athlete; `?range=` is admin all-athlete), `GET /api/sessions?history=1` (completed sessions + completed sets), `GET /api/badges` includes `bonusCount`, `GET /api/coach-catalog`, `GET /api/ratings/stats`, `GET|POST /api/feedback`, `POST /api/session-ratings`, `DELETE /api/exercises?id=` (uncompleted extras only), `GET /api/admin/analytics` are session-gated; analytics / admin feedback / household ratings / exercise-compare `?range=` also `requireAdmin`)
...
- Triggers:
  ...
  - Session marked complete → `checkAndAwardBadges` in the PUT (returned to the finish takeover, plus `{ bonus, bonusCount }` for bonus days) + recap (week/program folded in) + badge mail for streak/weight/perfect_week/total_workouts only
  ...
  - Scoreboard: Mondays only, to `WORKIT_SCOREBOARD_TO` (Kevin). Each athlete’s mail includes Best day / Total weight ranking, their Vs the house standing (weight + reps), the household table, plus the bonus honor roll (unique weeks)
