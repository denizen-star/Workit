...
- `app/home/page.tsx` — dashboard (scoreboard folded, **Vs the house** folded, **Your performance** folded after Vs the house (Summary / By exercise / By workout start folded), Achievements folded, Completed log folded under Weekly Progress). Week cards / **Completed** open that log. Bonus flag + unique-week tally when the week has a bonus day (`components/BonusFlag.tsx`). Optional flag (`components/OptionalFlag.tsx`): this-week warmup/cooldown counts + unique optional-week tally. Stat cards + progress charts show you / household avg of people who finished a workout in the last 7 days, **including you**, minus Test (`GET /api/stats` `household`; null if nobody trained). Week progress caps at 4. **Vs the house** (`GET /api/exercise-compare?period=7|30|all`): current athlete only; Best day / Total weight ranking plus two boards (best-day weight, best-day reps); hidden for Test. **Your performance** (`GET /api/athlete-performance?period=15|30|all`): you vs the last time you did that lift / that program day (gains, losses, linear hold/climb vs drop, perception avg); hidden for Test. Optional lbs sit in Total weight only. Enjoyment charts if the athlete has session ratings. Scoreboard honor roll from `GET /api/scoreboard` (`householdBonusHonor`, `householdOptionalHonor`)
...
- `app/api/*/route.ts` — one file per resource (`GET /api/scoreboard?period=7|30|all` includes bonus + optional honor rolls, `GET /api/stats` (includes `household`), `GET /api/exercise-compare?period=7|30|all` (current athlete; `?range=` is admin all-athlete), `GET /api/athlete-performance?period=15|30|all` (current athlete vs self; hidden for Test), `GET /api/sessions?history=1` (completed sessions + completed sets), `GET /api/badges` includes `bonusCount` / `optionalWeekCount` / `optionalCount`, `POST /api/optionals` (start/complete warmup or cooldown; stretch/core may skip the 10-minute wait with `circuitComplete`), `GET /api/coach-catalog`, `GET /api/ratings/stats`, `GET|POST /api/feedback`, `POST /api/session-ratings`, `POST /api/exercises` (log set, or `hardness` 1–5 on a completed set once), `DELETE /api/exercises?id=` (uncompleted extras only), `GET /api/admin/analytics` are session-gated; analytics / admin feedback / household ratings / exercise-compare `?range=` also `requireAdmin`)
