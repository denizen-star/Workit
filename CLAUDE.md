...
- `app/admin/page.tsx` — household users (Kevin-only); link to Analytics
...
- `app/api/*/route.ts` — one file per resource (`GET /api/scoreboard?period=7|30|all`, `GET /api/coach-catalog`, `GET /api/admin/analytics` are session-gated; analytics also `requireAdmin`)
