...
**Auth flow**: Session-based, not NextAuth. `lib/session.ts` issues/verifies an HS256 JWT (`jose`) in the `workit_session` cookie, signed with `AUTH_SECRET` (≥32 chars). `lib/auth.ts` wraps PIN hashing (scrypt), in-memory lockout (resets on redeploy), and `requireCurrentUser`/`requireAdmin`. `middleware.ts` gates every request except static assets, `/api/cron/*`, `/who`, `/api/auth/*`, and `GET /api/users`. Unauthenticated pages → `/who`; unauthenticated APIs → 401 JSON. User id `1` is the sole admin (`ADMIN_USER_ID`).
...
- `app/api/*/route.ts` — one file per resource
...
- Triggers:
  ...
  - Session marked complete → recap (week/program folded in) + badge mail for streak/weight/perfect_week/total_workouts
...
- Manual: `/admin/mail`, `npm run mail:release` (all household emails), `npm run mail:scoreboard`. `/document` **always** rewrites `lib/emails/currentRelease.ts` in Master Workit user-tone and runs `mail:release`.
