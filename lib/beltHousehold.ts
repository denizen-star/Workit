...
export async function householdBeltRows(householdId?: number | null) {
  ...
  const [users, locked] = await Promise.all([
    query(
      `SELECT id, name, coach_tone, gender FROM users WHERE pin_hash IS NOT NULL ${house.sql} ORDER BY name ASC`,
      house.params
    ),
    lockedWeeksByUserFromTable(householdId),
  ]);
  return (users.rows as { id: number; name: string; coach_tone?: string | null; gender?: string | null }[]).map((user) => {
    const lockedWeeks = locked.get(Number(user.id)) || 0;
    return {
      id: Number(user.id),
      name: user.name,
      ...progressFor(lockedWeeks, user.coach_tone, user.name, user.gender),
    };
  });
}
