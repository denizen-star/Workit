...
**Work-It:** Follow **`CLAUDE.md`** for routes, auth/middleware behavior, and API shape. Keep changes scoped; do not invent tables, env vars, or bypass `middleware.ts`/`requireAdmin` outside existing patterns.

## Bug Diagnosis — Read Real Data First
When reproducing a data bug (sessions, sets, badges, stats):
1. Trace the code path from UI (`app/workout/page.tsx` or `app/page.tsx`) → `fetch` → handler in `app/api/*/route.ts`.
2. Confirm IDs and payloads (user id, session id, week/day) match what the UI sends, and that auth (`getCurrentUser`/`requireCurrentUser`) resolves as expected.
3. Prefer a minimal repro against `npm run dev` before changing shared helpers like `lib/db.ts` or `lib/auth.ts`.